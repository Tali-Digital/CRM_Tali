...
const SortableCard = ({ card, client, tags, users, onEdit }: { key?: string | number, card: InternalTaskCard, client?: Client, tags: Tag[], users: UserProfile[], onEdit: (card: InternalTaskCard) => void }) => {
  ...
  const title = isCustom ? (card.title || 'Card Personalizado') : (client?.name || card.clientName || 'Cliente Desconhecido');
  const themeColor = isCustom ? 'stone' : (client?.themeColor || 'blue');
      ...
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            {isCustom && (
              <span className="text-[8px] font-bold uppercase tracking-widest text-stone-400 mb-0.5">Nota Interna</span>
            )}
...
export const InternalTasksView: React.FC<InternalTasksViewProps> = ({ companyId, lists, cards, clients, tags, users }) => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (overListId && activeCard.listId !== overListId) {
      ...
      await updateInternalTaskCard(activeId, { 
        listId: overListId,
            ...
            <div className="flex gap-6 h-full items-start min-w-max">
              ...
              {lists.length === 0 && (
                <div className="w-full h-64 flex flex-col items-center justify-center text-stone-400 border-2 border-dashed border-stone-200 rounded-3xl">
                  <p className="mb-4">Nenhuma lista de tarefas criada.</p>
                  <button 
                    onClick={() => setIsAddListOpen(true)}
