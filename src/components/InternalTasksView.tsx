      ...
      return (
        ...
        >
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {total > 0 && (
              <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[8px] font-bold shrink-0 ${completed === total ? (client.themeColor === 'blue' ? 'bg-blue-600 text-white' : 'bg-yellow-600 text-white') : (isDarkBg ? 'bg-white/20 text-white' : (client.themeColor === 'blue' ? 'bg-blue-50 text-blue-400 border border-blue-100' : 'bg-yellow-50 text-yellow-500 border border-yellow-100'))}`}>
                <CheckSquare size={8} />
                {completed}/{total}
              </div>
            )}
...
const SortableList = ({ list, cards, clients, tags, users, onEditCard, onQuickView, onSettings, onAddCard, onUpdateCard, viewMode, cardFilter, highlightedListId, highlightedCardId }: { key?: string | number, list: InternalTaskList, cards: InternalTaskCard[], clients: Client[], tags: Tag[], users: UserProfile[], onEditCard: (card: InternalTaskCard) => void, onQuickView: (card: InternalTaskCard) => void, onSettings: () => void, onAddCard: () => void, onUpdateCard: (cardId: string, data: Partial<InternalTaskCard>) => Promise<void>, viewMode: 'kanban' | 'list' | 'vertical', cardFilter: SectorCardFilter, highlightedListId: string | null, highlightedCardId: string | null }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`flex-1 flex ${viewMode === 'kanban' ? 'gap-6' : 'flex-col gap-8'} min-h-[100px]`}>
...
export const InternalTasksView: React.FC<InternalTasksViewProps> = ({ 
  ...
  const onEditCard = (card: InternalTaskCard) => {
    setEditingCard(card);
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (isActiveList) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        const newLists = arrayMove(lists, oldIndex, newIndex) as InternalTaskList[];
        newLists.forEach((list, index) => {
          if (list.order !== index) {
            updateInternalTaskList(list.id, { order: index });
          }
        });
      }
      return;
    ...
    if (overListId && activeCard.listId !== overListId) {
      ...
      let newChecklist = activeCard.checklist || [];
      ...
      const executeMove = (listId: string, order: number) => updateInternalTaskCard(activeId, { listId, order });
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        const newCards = arrayMove(listCards, oldIndex, newIndex) as InternalTaskCard[];
        newCards.forEach((card, index) => {
          if (card.order !== index) {
            updateInternalTaskCard(card.id, { order: index });
          }
        });
      }
    }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <Modal isOpen={isAddListOpen} onClose={() => setIsAddListOpen(false)} title="Nova Lista">
      ...
      {editingList && (
        <ListSettingsModal 
          ...
          list={editingList} 
          onUpdate={updateInternalTaskList}
          onDelete={deleteInternalTaskList}
        />
      )}

      {editingCard && (
        <EditInternalTaskCardModal 
          isOpen={!!editingCard} 
