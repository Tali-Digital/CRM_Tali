          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {filteredClients.map(client => {
                const commCard = commercialCards.find(c => c.clientId === client.id);
                const commList = commCard ? commercialLists.find(l => l.id === commCard.listId) : null;
                
                const finCard = financialCards.find(c => c.clientId === client.id);
                const finList = finCard ? financialLists.find(l => l.id === finCard.listId) : null;

                const opCard = operationCards.find(c => c.clientId === client.id);
