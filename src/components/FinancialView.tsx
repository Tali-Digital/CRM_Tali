...
const SortableList = ({ list, cards, clients, tags, users, onEditCard, onQuickView, onSettings, onAddCard, onUpdateCard, viewMode, cardFilter, highlightedListId, highlightedCardId }: { key?: string | number, list: FinancialList, cards: FinancialCard[], clients: Client[], tags: Tag[], users: UserProfile[], onEditCard: (card: FinancialCard) => void, onQuickView: (card: FinancialCard) => void, onSettings: () => void, onAddCard: () => void, onUpdateCard: (cardId: string, data: Partial<FinancialCard>) => Promise<void>, viewMode: 'kanban' | 'list' | 'vertical' | 'calendar', cardFilter: SectorCardFilter, highlightedListId: string | null, highlightedCardId: string | null }) => {
...
export const FinancialView: React.FC<FinancialViewProps> = ({ 
  ...
  const [quickViewSector, setQuickViewSector] = useState<'commercial' | 'financial' | 'operation' | 'internal'>('financial');
  ...
  useEffect(() => {
    if (jumpToCard && (jumpToCard.sector === 'financial' || jumpToCard.sector === 'integracao') && jumpToCard.id) {
      const targetCard = allFinancialCards.find(c => c.id === jumpToCard.id);
      if (targetCard) {
  ...
  }, [jumpToCard, allFinancialCards, onClearJump]);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (isActiveList) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        const newLists = arrayMove(lists, oldIndex, newIndex) as FinancialList[];
        newLists.forEach((list, index) => {
          if (list.order !== index) {
            updateFinancialList(list.id, { order: index });
          }
        });
      }
      return;
    ...
    if (overListId && activeCard.listId !== overListId) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        const newCards = arrayMove(listCards, oldIndex, newIndex) as FinancialCard[];
        newCards.forEach((card, index) => {
          if (card.order !== index) {
            updateFinancialCard(card.id, { order: index });
          }
        });
      }
    }
  ...
  const onQuickView = (card: any) => {
    ...
    setQuickViewSector('financial');
  };

  const handleAddList = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newListName.trim()) return;
    await addFinancialList({
    ...
    return (
          ...
          <div className="flex-1 min-h-0 bg-white rounded-3xl overflow-hidden shadow-sm border border-stone-200">
            <CalendarDashboardView 
              allCards={activeCards
                ...
                .map(c => ({ ...c, sector: 'integracao' }))}
              clients={clients}
              tags={tags}
              users={users}
              onCardClick={(card) => setQuickViewCard(card as any)}
              sector="financial"
              onEdit={() => editingCard && setEditingCard(editingCard)}
        ...
        <form onSubmit={handleAddList} className="space-y-4">
          <div className="space-y-2">
            ...
            <input 
              ...
              placeholder="Ex: A Receber, Faturado, etc."
            />
          </div>
      ...
      {editingList && (
        <ListSettingsModal 
          ...
          list={editingList} 
          onUpdate={updateFinancialList}
          onDelete={deleteFinancialList}
        />
      )}
