  ...
  const handleSearchCnpj = async () => {
    ...
    if (cleanCnpj.length !== 14) {
      Swal.fire({
        icon: 'error',
        title: 'CNPJ inválido',
        text: 'Digite os 14 números do CNPJ.'
      });
      return;
    }
    ...
    try {
      // Primary API: minhareceita.org (more accurate for current partners, allows CORS)
      ...
      if (data.qsa && Array.isArray(data.qsa)) {
        ...
        if (newOwners.length > 0) {
          ...
          Swal.fire({
            icon: 'info',
            title: 'Nenhum sócio',
            text: 'Não foram encontrados sócios no quadro societário.'
          });
        }
      } else {
        // Fallback to cnpj.ws
        ...
        if (fallbackData.socios && Array.isArray(fallbackData.socios)) {
          const newOwners = fallbackData.socios.map((socio: any) => {
            const formattedName = toTitleCase(socio.nome);
            const role = socio.qualificacao_socio?.descricao?.trim() || '';
            return {
              name: formattedName,
              role: role ? toTitleCase(role) : '',
              instagram: ''
          ...
          if (newOwners.length > 0) {
            ...
            if (fallbackData.estabelecimento?.logradouro) {
              ...
              fetchedAddress = [
                ...
                est.complemento,
                est.bairro,
                est.cidade?.nome ? `${est.cidade.nome} - ${est.estado?.sigla || ''}` : null,
          ...
          Swal.fire({
            icon: 'error',
            title: 'Erro',
            text: 'Não foi possível carregar os sócios desse CNPJ.'
          });
        }
      }
    } catch (error) {
      ...
      Swal.fire({
        ...
        text: 'Houve um erro ao buscar o CNPJ. Tente novamente.'
      });
    } finally {
          ...
          <div className="flex flex-wrap items-center gap-2 flex-1">
            ...
            <div className="relative shrink-0 animate-in fade-in zoom-in duration-200" ref={progressFilterRef}>
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition-all shadow-md active:scale-95 text-xs font-semibold border ${quickFilter !== 'active' && quickFilter !== 'all'
                    ...
                    {[
                      ...
                      f.group ? (
                        ...
                        <button
                          ...
                          className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center justify-between ${quickFilter === f.key
                        ...
                        >
                          ...
                          <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${quickFilter === f.key
              ...
              {processedProspects.map((p, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-2xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 flex flex-col overflow-hidden relative cursor-pointer group ${isSelected
              ...
              {editingProspect && (
                <div className="flex flex-nowrap overflow-x-auto border-b border-gray-100 bg-gray-50/50 p-2 sm:p-4 gap-2 sm:gap-3 justify-start items-center" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                  <button
                    ...
                    className={`shrink-0 px-5 py-3 sm:px-6 sm:py-3 text-[12px] sm:text-sm font-bold rounded-2xl transition-all duration-200 flex items-center gap-2 whitespace-nowrap ${activeTab === 'dados'
                  ...
                  <button
                    ...
                    className={`shrink-0 px-5 py-3 sm:px-6 sm:py-3 text-[12px] sm:text-sm font-bold rounded-2xl transition-all duration-200 flex items-center gap-2 whitespace-nowrap ${activeTab === 'ia'
                  ...
                  <button
                    ...
                    className={`shrink-0 px-5 py-3 sm:px-6 sm:py-3 text-[12px] sm:text-sm font-bold rounded-2xl transition-all duration-200 flex items-center gap-2 whitespace-nowrap ${activeTab === 'instagram'
                  ...
                  <button
                    ...
                    className={`shrink-0 px-5 py-3 sm:px-6 sm:py-3 text-[12px] sm:text-sm font-bold rounded-2xl transition-all duration-200 flex items-center gap-2 whitespace-nowrap ${activeTab === 'calculadora'
                          ...
                          <div className="md:col-span-3 bg-blue-50/10 border border-blue-100 rounded-3xl p-6 shadow-sm border-t-4 border-t-blue-600 transition-all hover:shadow-md">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                              ...
                              <div className="space-y-1 md:col-span-3 lg:col-span-3">
                                <label className="text-xs font-bold text-gray-500 uppercase ml-1 flex items-center justify-between">
                                  <span>CNPJ</span>
                                  {isFetchingCnpj && <Loader2 size={12} className="animate-spin text-blue-500" />}
                                </label>
                                <div className="relative">
                                  <input
                                    type="text"
                                    className="w-full pl-4 pr-32 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-900 outline-none transition-all text-sm font-medium"
                                  ...
                                  <button
                                    ...
                                    disabled={!formData.cnpj || isFetchingCnpj}
                                    className="absolute right-1 top-1 bottom-1 bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 rounded-lg text-xs font-bold transition-colors disabled:opacity-50"
                                  >
                                    Buscar Sócios
                                  </button>
                                </div>
                                <a
                                  href={`https://www.google.com/search?q=CNPJ+${encodeURIComponent(formData.clinicName || '')}+${encodeURIComponent(formData.location || '')}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                ...
                                >
                                  <Search size={10} /> Pesquisar CNPJ no Google
                                </a>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="md:col-span-3 bg-pink-50/10 border border-pink-100 rounded-3xl p-6 shadow-sm border-t-4 border-t-pink-500 transition-all hover:shadow-md">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                              <div className="space-y-1">
                                ...
                                />
                                <a
                                  href={`https://www.google.com/search?q=Instagram+${encodeURIComponent(formData.clinicName || '')}+${encodeURIComponent(formData.location || '')}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-[10px] font-bold text-pink-600 hover:underline flex items-center gap-1 mt-1 ml-1 w-max"
                                  title="Pesquisar Instagram no Google"
                                >
                                  <Search size={10} /> Pesquisar Instagram no Google
                                </a>
                              </div>
                                    ...
                                    {ownersList.map((owner, idx) => (
                                      <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-5 items-start relative group">
                                        <div className="space-y-1">
                                          ...
                                          />
                                          {owner.role && (
                                            <span className="text-[10.5px] font-bold text-blue-800 !-mt-0.2 ml-1 block leading-none">
                                              {owner.role}
                                            </span>
                                          )}
                                        </div>
                      ...
                      <div className="space-y-5 animate-in fade-in duration-200">
                        ...
                        <div className="flex gap-3 border-b border-gray-100 pb-4">
                          <button
                            ...
                            className={`px-5 py-2 text-xs font-bold rounded-xl transition-all flex items-center gap-2 ${aiSubTab === 'analise'
                          ...
                          <button
                            ...
                            className={`px-5 py-2 text-xs font-bold rounded-xl transition-all flex items-center gap-2 ${aiSubTab === 'preenchimento'
