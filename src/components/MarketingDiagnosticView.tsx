...
import { 
  Search, Brain, Map, Activity, CheckCircle2, ChevronRight, Loader2, Sparkles, AlertTriangle
} from 'lucide-react';
...
export const MarketingDiagnosticView: React.FC<Props> = ({ companyId }) => {
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [companyId]); // Removed selectedProspect and diagnosticData from deps to avoid infinite loops
                ...
                <tr key={idx} className="border-b border-gray-800">
                  ...
                  {diagnosticData.concorrentes?.map((c: any, i: number) => (
                    <td key={i} className="py-3 px-4 text-center text-sm">
                      {row.isCheck ? (
                        c[row.key] ? <span className="text-green-500">✔️</span> : <span className="text-red-500">❌</span>
                      ) : (
                        c[row.key]
                      )}
                    </td>
  ...
  return (
    <div className="flex h-screen bg-stone-50 p-2 gap-2">
      {/* Left Sidebar */}
      <div className="w-80 bg-white rounded-3xl border border-stone-200 flex flex-col overflow-hidden shadow-sm">
        <div className="p-5 border-b border-stone-100">
          ...
          <p className="text-xs text-stone-500 mt-1">Prospecções Presenciais marcadas</p>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {filteredProspects.map(p => (
            <div 
              ...
              className={`p-3 rounded-xl cursor-pointer transition-all ${selectedProspect?.id === p.id ? 'bg-[#5271FF] text-white shadow-md' : 'hover:bg-stone-50'}`}
            >
          ...
          {filteredProspects.length === 0 && (
            <div className="text-center p-8 text-stone-400">
              ...
              <p className="text-sm font-medium">Nenhuma prospecção presencial encontrada.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-3xl border border-stone-200 overflow-hidden shadow-sm flex flex-col">
        {selectedProspect ? (
          <div className="h-full flex flex-col bg-[#0d0f19]">
            <div className="p-4 border-b border-gray-800 flex items-center justify-between bg-[#1a1d2d]">
              ...
              {!diagnosticData && (
                <button 
                  ...
                  className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50"
                >
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {!diagnosticData ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400 text-center px-4">
                  <div className="bg-gray-800/50 p-6 rounded-2xl max-w-md border border-gray-800">
                    <Sparkles className="mx-auto text-indigo-500 mb-4" size={40} />
                    <h3 className="text-white font-bold text-lg mb-2">Inteligência Artificial Pronta</h3>
                    <p className="text-sm mb-4">Clique no botão acima para gerar uma análise profunda baseada nos dados do CRM ({selectedProspect.gmnRating}★, {selectedProspect.gmnReviewsCount} avaliações).</p>
                    <div className="bg-gray-900 p-3 rounded-lg flex items-center justify-between border border-gray-700">
                      <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Custo Estimado API:</span>
                      <span className="text-sm font-black text-green-400">~ $0.001 USD</span>
                    </div>
                  </div>
                </div>
              ) : (
