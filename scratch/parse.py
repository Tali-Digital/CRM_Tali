...
tsv = """Ordem	Responsável Talí	Cidade/Bairro - Estado	Nome Clínica	Instagram Clínica	GMN	Site	Nome Dono	Instagram Dono	Seguiu o dono?	Colaboradores	Tamanho	Idade	Nota / Qtd de Avaliações	Observações Importantes	Status	Resposta?	Último Follow up	Abordagem usada	Data 1º Contato	Semana
...
2	Diogo	Águas Claras - DF	Odonto Aguas Claras	https://www.instagram.com/odontoaguasclaras/	https://www.google.com/maps?cid=10289450762552572900	https://www.odontoaguasclaras.com.br/	"Carlos Eduardo Silva Vale
Heverton de Alencar Silva Ferreira 
Mariana Barroso Coelho "	https://www.instagram.com/marianab.coelho/	Sim		1 Cadeira	17 anos	4,9 / 225		Mensagem Enviada	FALSE		v2	20-mai.-2026	Semana 1
...
11	Helenilton	Águas Claras - DF	Lírios Odontologia	https://www.instagram.com/dra.alineolive/	Lírios Odontologia - Dra. Aline Olive	https://draalineolive.com/	ALINE OLIVE DE ARAUJO JANUARIO	https://www.instagram.com/dra.alineolive/	Sim		1 Cadeira			"- SEM SITE ATIVO
- Vídeo de Antes e depois muito bem pensado com imagem da pessoa e sorriso anterior no canto da tela"	Mensagem Enviada	FALSE		V2	6-mai.-2026	Semana 2
...
13	Helenilton	Águas Claras - DF	Atually Odontologia Especializada	https://www.instagram.com/atually.odontologia/	https://maps.google.com/?cid=15429181756021543609	https://atuallyodontologia.com.br/	BRUNA MOREIRA COELHO, JHYMES DE SOUZA RODRIGUES	"https://www.instagram.com/dra.bruna_moreira?igsh=MTBkcWh5czVkdDZyMQ%3D%3D&utm_source=qr

https://www.instagram.com/dr.jhymes_rodrigues/"	Sim		3+ Cadeiras 			Coloquei o Instagram dos dois Donos, segui os dois também, vai que com um deles da certo	Mensagem Enviada	FALSE			6-mai.-2026	Semana 2
...
19	Helenilton	Águas Claras - DF	IMP Odonto	https://www.instagram.com/impodonto/	https://maps.google.com/?cid=540869271782453575	http://impodonto.com.br/	RICARDO FABRIS PAULIN, LIANA BONFIM MISSON PAULIN	"https://www.instagram.com/drricardopaulin/
"	Solicitado		3+ Cadeiras 			Ja tem Agência (https://insitemarketing.digital/)	Mandar mensagem	FALSE		"Oi, Dr. Ricardo, tudo bem?

Me chamo Helenilton Alves, sou um dos fundadores da Talí Digital. Vi o perfil da IMP Odonto no Google, entrei aqui no Instagram e achei incrível a estrutura do trabalho de vocês aí no edifício E-Business. Dá para ver de cara que é uma operação muito séria na região, e foi justamente por notar esse padrão que fiz questão de entrar em contato.
...
23	Helenilton	Águas Claras - DF	Orthos Odontologia	https://www.instagram.com/orthosbrasilia	https://maps.google.com/?cid=17602167728930593026	https://orthosodonto.com.br/	Dra Mariella Salgado	https://www.instagram.com/dra.mariellasalgado	Solicitado		3+ Cadeiras 			"Site fora do ar
Segui o Welss e o Sérgio Marra"	Mandar mensagem	FALSE			19-mai.-2026	Semana 1
...
27	Helenilton														Mandar mensagem	FALSE			20-mai.-2026	Semana 1
...
output = []
for r in reader:
    if not r.get('Nome Clínica') and not r.get('Responsável Talí'):
        continue
    
    ownerName = r.get('Nome Dono', '').replace('\\n', ', ')
    ownerInstagram = r.get('Instagram Dono', '').replace('\\n', ', ')
    observations = r.get('Observações Importantes', '').replace('\\n', ' ')
    approachUsed = r.get('Abordagem usada', '').replace('\\n', ' ')
    
    output.append({
        'order': int(r.get('Ordem') or 0),
        'responsible': r.get('Responsável Talí', ''),
        'location': r.get('Cidade/Bairro - Estado', ''),
        'clinicName': r.get('Nome Clínica', ''),
        'clinicInstagram': r.get('Instagram Clínica', ''),
        'gmn': r.get('GMN', ''),
        'site': r.get('Site', ''),
        'ownerName': ownerName,
        'ownerInstagram': ownerInstagram,
        'followedOwner': r.get('Seguiu o dono?', ''),
        'collaborators': r.get('Colaboradores', ''),
        'size': r.get('Tamanho', ''),
        'age': r.get('Idade', ''),
        'gmnRating': r.get('Nota / Qtd de Avaliações', ''),
        'observations': observations,
        'status': r.get('Status', ''),
        'hasAnswered': r.get('Resposta?') in ['TRUE', 'Sim'],
        'lastFollowUp': r.get('Último Follow up', ''),
        'approachUsed': approachUsed,
        'firstContactDate': r.get('Data 1º Contato', ''),
        'week': r.get('Semana', ''),
        'currentStep': 1
    })
...
for item in output:
    ...
    for k, v in item.items():
        if isinstance(v, str):
            ...
            code += f"{k}: {'true' if v else 'false'}, "
        else:
            code += f"{k}: {v}, "
    code += "},\\n"
code += "    ];"

with open("d:\\\\- - - - Antigravit Projetos\\\\CRM_Tali\\\\scratch\\\\data_code.txt", "w", encoding='utf-8') as f:
    f.write(code)
print("Done!")
