...
export default function GerenciadorModelosModal({ onClose }: Props) {
  ...
  const saveModelos = async (novosModelos: ModeloContrato[]) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (isConfirmed) {
      saveModelos(modelos.filter(m => m.id !== id));
      Swal.fire({
        title: 'Excluído!',
  ...
  const handleStartEdit = (m: ModeloContrato) => {
    setEditingId(m.id);
    setEditNome(m.nome);
  };
  ...
  const handleSaveEdit = (id: string) => {
    ...
    saveModelos(modelos.map(m => m.id === id ? { ...m, nome: editNome } : m));
    setEditingId('');
  };
